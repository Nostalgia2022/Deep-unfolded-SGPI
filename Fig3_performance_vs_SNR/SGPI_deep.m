...
% step_size=[4.1379, 3.6770, 1.0963, 0.7211, 0.3098, 1.4435, 0.2425, 2.2332, 0.4196...
    % 0.2225, 2.0256, 0.1964, 1.6128, 0.4104, 0.1825, 1.9499, 0.4174, 0.3024, 0.1650, 2.1184];
...
Convergence_tot=zeros(I_out,40,I_in);
Convergence_com=zeros(I_out,40,I_in);
Convergence_sen=zeros(I_out,40,I_in);
stepsize_all=zeros(40,3);

load dataformatbal_snr10_iter40.mat
stepsize_all(:,1)=step_size;
load dataformatbal_snr15_iter40.mat
stepsize_all(:,2)=step_size;
load dataformatbal_snr20_iter40.mat
stepsize_all(:,3)=step_size;
...
for k_par=1 :I_out*I_in
    ...
    Pt=db2pow(8+weight*2); %dBm
    if 8+weight*2<13
        step_size=stepsize_all(:,1);
    elseif 8+weight*2>17
        step_size=stepsize_all(:,3);
    else
        step_size=stepsize_all(:,2);
    end
